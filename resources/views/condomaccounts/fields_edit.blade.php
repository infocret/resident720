...
{!! Form::label('bank_id', 'Banco:') !!}	 
 ...
 <option value="">Seleccione Banco</option>   
    @foreach($bancos as $key => $value)            
        <option value="{{$key}}"
        {{$propaccount->bank_id == $key ? 'selected="selected"' : '' }}
           >{{ $value }}
        </option>            
    @endforeach 
</select> 
...
{!! Form::label('lbankaccount_id', 'Cuenta:') !!}	 
 ...
 <option value="">Seleccione Cuenta</option>
    @foreach($bankaccounts as $key => $value)            
        <option value="{{$key}}"
        {{$propaccount->bcta_id == $key ? 'selected="selected"' : '' }}
           >{{ $value }}
        </option>            
    @endforeach 
 </select>
...
<div class="form-group col-sm-6">   
 ...
 <option value="">Seleccione Chequera</option>
    @foreach($bankchbooks as $key => $value)            
        <option value="{{$key}}"
        {{$propaccount->bcta_id == $key ? 'selected="selected"' : '' }}
           >{{ $value }}
        </option>            
...
<div class="form-group col-sm-6">
    ...
    {!! Form::text('bank_agreement', $propaccount->convenio, ['class' => 'form-control']) !!}
</div>
